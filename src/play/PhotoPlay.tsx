  ...
  return <div className="play-app">
    ...
    <main className="play-main">
      ...
      {!mission ? <>
        ...
        <div className="play-badges" aria-label="Earned photo mission badges">{score.badges.map(b=><span key={b}>✦ {b}</span>)}</div><div className="play-bottom-note"><span>✳</span><p>Each completed mission earns 100 XP. The <button onClick={()=>onNavigate('crates')}>50% volume goal</button> comes from the fill estimates you confirm in Crates.</p><button onClick={()=>onNavigate('dashboard')}>Task progress ↗</button></div>
      </> : <>
