...
export function PhotoPlay({onNavigate, initialCrateId, onOpenCrate}:{onNavigate:(tab:Tab)=>void;initialCrateId?:string|null;onOpenCrate:(id:string)=>void}) {
  ...
  function beginRound() {
    ...
    const value=createMission({id:editingId||crypto.randomUUID(),title:kind==='crate'&&currentCrate?`Sort ${currentCrate.code}`:template.title,area:area.trim()||currentCrate?.location||template.area,kind,crateId:kind==='crate'&&currentCrate?currentCrate.id:null,plannedMinutes:minutes},Date.now())
    let created=false
    ...
    <main className="play-main">
      ...
      {!mission ? <>
        <section className="play-hero">
          ...
          <div className="play-hero-photo"><img src="/evidence/2026-09-09/IMG_1930.jpg" alt="Your storage racks behind the white parking-clearance line" /><div className="play-photo-shade"/><span className="play-photo-caption">YOUR GARAGE · SEPT 9 REFERENCE</span><div className="play-photo-reticle"><i/><span>ONE SMALL AREA<br /><b>YOU’VE GOT THIS.</b></span><i/></div><div className="play-photo-sticker"><span>YOUR NEXT MISSION</span><b>Choose one<br />area to clear.</b><span>+100 XP / FINISHED MISSION</span></div></div>
        </section>
        {open.length>0&&<section className="play-resume"><div><span className="play-kicker">YOUR SAVED MISSION</span><h2>{open[0].title}</h2><p>{open[0].area} · {open[0].phase==='before'?'Before photo and start':open[0].phase==='active'?'Mission in progress':'After photo and finish'}</p></div><button className="play-primary" onClick={()=>openRound(open[0].id)}>Continue mission →</button></section>}
        <section className="play-selection" ref={selectionRef}>
          <div className="play-section-heading"><div><span className="play-kicker">CHOOSE A MISSION</span><h2>What will you work on?</h2></div><span>Finish one mission at a time. Keep both car spaces clear.</span></div>
          ...
          <div className="play-setup">
            ...
            <button className="play-primary" disabled={open.length>0&&!editingId} onClick={beginRound}>{editingId?'Save changes & take before photo':'Take before photo'} <CameraIcon /></button>
          </div>
          {open.length>0&&!editingId&&<p className="play-fine">Continue the saved mission above. Finish it before starting a new one.</p>}
        </section>
        <section className="play-wins"><div className="play-section-heading"><div><span className="play-kicker">YOUR BEFORE-AND-AFTER PHOTOS</span><h2>Completed missions</h2></div><span>{finished.length} missions finished · {score.roundsToNextLevel} to level {score.level+1}</span></div>{!finished.length?<div className="play-empty-wall"><span>✦</span><p>Your completed missions will appear here.<br /><b>Each one saves your before and after photos.</b></p></div>:<div className="play-win-grid">{finished.map(m=><button key={m.id} className="play-win-card" onClick={()=>openRound(m.id)}><div><img src={m.beforePhoto!} alt={`Before: ${m.area}`} loading="lazy"/><img src={m.afterPhoto!} alt={`After: ${m.area}`} loading="lazy"/><span>+100 XP</span></div><h3>{m.title}</h3><p>{m.summary}</p><small>{new Date(m.completedAt!).toLocaleDateString()} · Compare the photos ↗</small></button>)}</div>}</section>
        <div className="play-badges" aria-label="Earned photo mission badges">{score.badges.map(b=><span key={b}>✦ {b}</span>)}</div><div className="play-bottom-note"><span>✳</span><p>Each completed mission earns 100 XP. The <button onClick={()=>onNavigate('crates')}>50% volume goal</button> comes from the fill estimates you confirm in Crates.</p><button onClick={()=>onNavigate('dashboard')}>Previous game stats ↗</button></div>
      </> : <>
        <div className="play-round-heading"><button className="play-back" onClick={goLobby}>← All missions</button><span>{mission.area}</span></div>
