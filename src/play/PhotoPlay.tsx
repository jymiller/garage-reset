...
export function PhotoPlay({onNavigate, initialCrateId, onOpenCrate}:{onNavigate:(tab:Tab)=>void;initialCrateId?:string|null;onOpenCrate:(id:string)=>void}) {
  ...
  return <div className="play-app">
    <header className="play-nav">
      ...
      <button type="button" className="play-level" onClick={()=>onNavigate('score')} aria-label="Open points score"><span>LV {score.level}</span><b>{score.points} points</b><span className="play-score-arrow" aria-hidden="true">→</span></button>
    </header>
    <main className="play-main">
      ...
      {!mission ? <>
        {rewards ? <section className="play-player-setup" aria-labelledby="play-player-heading">
          ...
          <div className="play-player-choice"><label htmlFor="play-player">Player for this mission</label><select id="play-player" value={setupPlayer?.id ?? ''} onChange={event=>choosePlayer(event.target.value)}><option value="" disabled={Boolean(editingEntry)}>{editingId && !editingEntry ? 'No player assigned' : 'Choose a player'}</option>{players.map(player=><option key={player.id} value={player.id}>{player.name}</option>)}</select><button type="button" className="play-text-button" onClick={()=>onNavigate('score')}>{players.length ? 'Players & points →' : 'Add players in Score →'}</button></div>
          <p className="play-player-note">Completing a mission earns 100 points. Dollar values come later.</p>
        </section> : <section className="play-reward-note"><div><h2>Play for points.</h2><p>You can start a mission now. Choose your name in Score when you want the points credited to you.</p></div><button type="button" className="play-secondary" onClick={()=>onNavigate('score')}>Choose your name →</button></section>}
