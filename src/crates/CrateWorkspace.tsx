...
export function CrateWorkspace({ onNavigate, initialCrateId, initialStep, onPlayCrate, labelCode }: { onNavigate: (tab: Tab) => void; initialCrateId?: string|null; initialStep?: 'locate'|'sort'|'repack'; labelCode?: string|null; onPlayCrate?: (id:string)=>void }) {
  ...
  return <div className="crate-app">
    ...
    <main className="crate-main">
      <section className="crate-label-kit"><div><strong>Label first. Sort one box at a time.</strong><p>Each box gets a permanent ID and QR code. Print, cut out, and tape the labels to your boxes.</p></div><a href="/print/garage-container-labels.pdf" target="_blank" rel="noreferrer">Download 32 blank labels ↓</a><small>C-001–C-032 · 8 per Letter page · Print at 100% / actual size. Match any existing IDs before sticking.</small></section>
      {labelCode && !scannedOpened && !scanned && <section className="crate-scan-result" role="status"><span>SCANNED CONTAINER</span><h2>{labelCode}</h2>{workspace.status === 'connecting' ? <p>Looking up this label…</p> : workspace.status !== 'shared' ? <p>Connect to the shared workspace to check this ID before registering it. Your existing draft is kept.</p> : <><p>This label isn’t registered yet. Give this box a short name and shelf location to start.</p><button className="crate-primary" onClick={() => setAdding(true)}>Register {labelCode} →</button></>}</section>}

      <section className="crate-hero">
        <div><span className="crate-eyebrow">THE HALF-AS-MUCH PROJECT</span><h1>Label a box.<br /><em>Record what’s inside.</em></h1><p>Plan on your laptop. Open one crate on your phone.<br />Keep both car spaces clear, every session.</p></div>
      ...
      <div className={`crate-workbench ${crate ? 'has-selection' : ''}`}>
        ...
        <section className="crate-detail">
          {!crate ? <div className="crate-welcome"><span className="crate-eyebrow">START HERE</span><h2>{data.crates.length ? 'Choose a container to sort.' : 'Add your first container.'}</h2><p>Label it <strong>{labelCode && !scannedOpened ? labelCode : "C-001"}</strong>. Record where it lives, take a photo, and describe what’s inside. We’ll turn that into a contents list and a repacking plan.</p><ol><li><b>Locate</b><span>ID, shelf, owner and starting volume.</span></li><li><b>Open & sort</b><span>Photograph, describe, decide and assign destinations.</span></li><li><b>Repack</b><span>Record its new fill and confirm the parking floor is clear.</span></li></ol><button className="crate-primary" disabled={!registrationReady} onClick={()=>setAdding(true)}>{data.crates.length ? 'Register another container →' : 'Register the first crate →'}</button><p className="crate-fine">Phone: use the same app address as your laptop. Use your keyboard’s dictation to describe contents; add one item or group per line.</p></div> : <>
            <button className="crate-back" onClick={()=>{setScannedOpened(true);setSelected(null)}}>← All containers</button>
            <div className="crate-detail-title"><div><span className="crate-eyebrow">{crate.code} · {crate.owner || 'OWNER NOT ASSIGNED'}</span><h2>{crate.name}</h2><p>{crate.location || 'Record the shelf location below.'}</p></div><div className="crate-title-actions">{onPlayCrate&&<button className="crate-primary" onClick={()=>onPlayCrate(crate.id)}>▶ Play this crate</button>}<button className="crate-label-button" onClick={()=>setShowLabel(true)}>QR label ↗</button></div></div>
...
function NewCrate({locked,existingCodes,onClose,onCreate,initialCode,blocked}:{blocked:boolean;initialCode?:string|null;locked:boolean;existingCodes:string[];onClose:()=>void;onCreate:(crate:Crate)=>boolean}) {
