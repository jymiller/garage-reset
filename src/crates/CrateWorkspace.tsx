...
export function CrateWorkspace({ onNavigate, initialCrateId, initialStep, onPlayCrate, labelCode, onCapture, onLabelQuest }: { onNavigate: (tab: Tab) => void; onLabelQuest: (code?: string) => void; onCapture: (kind: 'crate'|'parking'|'measurement'|'placement', crateId?:string|null) => void; initialCrateId?: string|null; initialStep?: 'locate'|'sort'|'repack'; labelCode?: string|null; onPlayCrate?: (id:string)=>void }) {
  ...
  function moveItem(item: ContentItem, destinationId: string) {
    ...
    setNotice('Item moved. Update how full both containers are after moving the item.')
  }
  ...
  return <div className="crate-app">
    ...
    <main className="crate-main">
      ...
      {labelCode && !scannedOpened && !scanned && <section className="crate-scan-result" role="status"><span>SCANNED CONTAINER</span><h2>{labelCode}</h2>{workspace.status === 'connecting' ? <p>Looking up this label…</p> : workspace.status !== 'shared' ? <p>Connect to the shared workspace to check this ID before registering it. Your existing draft is kept.</p> : <><p>Start with photos of this box and what’s inside. No measurements needed.</p>{/^C-(?!000)\d{3}$/.test(labelCode.toUpperCase()) && <button className="crate-primary" onClick={() => onLabelQuest(labelCode)}>Label & photograph →</button>}<details><summary>Full inventory setup (optional)</summary><button className="crate-secondary" onClick={() => setAdding(true)}>Register {labelCode} →</button></details></>}</section>}
      ...
      {!crate && (!labelCode || scannedOpened) && <section className="crate-quick-start"><GarageIcon name="crate"/><div><span>START SMALL</span><h1>Just show us a crate.</h1><p>Take a photo where it lives. No label, measurements or inventory needed.</p><button className="crate-primary" onClick={()=>onLabelQuest()}>Label & photograph →</button><button className="crate-secondary" onClick={()=>onCapture('crate')}>Take a crate photo <span aria-hidden="true">→</span></button><small>A closed box is fine. Open the lid only if it’s easy.</small>{(data.observations ?? []).some(item=>item.kind==='crate') && <button className="crate-start-text" onClick={()=>onNavigate('observations')}>See your crate photos →</button>}</div></section>}
      {!crate && photographedLabels.length>0 && <section className="crate-photo-catalog"><h2>Your photographed crates</h2><p>Photos and notes saved by label. Full inventory can wait.</p><div>{photographedLabels.map(code=>{const photos=(data.observations??[]).filter(photo=>photo.labelCode===code);const latest=photos.slice().sort((a,b)=>b.createdAt-a.createdAt)[0];return <button key={code} onClick={()=>onLabelQuest(code)}><img src={latest.photo} alt={`${code} latest view`}/><span><b>{code}</b><strong>{findCrateByCode(data.crates,code)?.name || latest.location || 'Open photos & contents'}</strong><small>{photos.length} photos · {photos.some(photo=>photo.photoRole==='contents')?'Contents photographed':'Contents still to explore'}</small></span></button>})}</div></section>}
      <div className={`crate-workbench ${crate ? 'has-selection' : ''}`}>
        <aside className="crate-roster">
          <div className="crate-roster-heading"><h2>Registered containers</h2><button disabled={!registrationReady} onClick={() => setAdding(true)}>+ Register</button></div>
          ...
          {!data.crates.length && <div className="roster-empty"><GarageIcon name="crate" className="crate-empty-icon"/><p>No registered containers yet.<br />You can take photos first.</p></div>}
        ...
        <section className="crate-detail">
          {!crate ? <details className="crate-register-later"><summary>Ready to record what’s inside?</summary><p>Register a container when you want to track its contents and how much space it uses. You can label it later.</p><button className="crate-primary" disabled={!registrationReady} onClick={()=>setAdding(true)}>Register a container →</button></details> : <>
            <button className="crate-back" onClick={()=>{setScannedOpened(true);setSelected(null)}}>← All containers</button>
            <div className="crate-detail-title"><div><span className="crate-eyebrow">{crate.code} · {crate.owner || 'OWNER NOT ASSIGNED'}</span><h2>{crate.name}</h2><p>{crate.location || 'Record the shelf location below.'}</p></div><div className="crate-title-actions">{/^C-(?!000)\d{3}$/.test(crate.code.toUpperCase()) && <button className="crate-primary" onClick={()=>onLabelQuest(crate.code)}>Label & photograph</button>}<button className="crate-label-button" onClick={()=>onCapture('crate',crate.id)}>Add a helpful photo</button>{onPlayCrate&&<button className="crate-label-button" onClick={()=>onPlayCrate(crate.id)}>▶ Play this crate</button>}<button className="crate-label-button" onClick={()=>setShowLabel(true)}>QR label ↗</button></div></div>
            ...
            {notice && <p className="crate-notice" role="status">{notice} {workspace.dirty ? 'Waiting to save with the family.' : workspace.status==='shared' ? 'Saved with the family.' : 'Check the save status above.'}</p>}

          </>}
        </section>
      </div>
      <details className="crate-optional-tools"><summary>Volume goals & planning tools</summary>
      <section className="crate-hero">
        <div><span className="crate-eyebrow">THE HALF-AS-MUCH PROJECT</span><h2>Storage progress.<br /><em>When you’re ready.</em></h2><p>Plan on your laptop. Open one crate on your phone.<br />Keep both car spaces clear, every session.</p></div>
      ...
      <details className="crate-optional-tools"><summary>Print labels (optional)</summary>
      <section className="crate-label-kit"><div><strong>Labels when you’re ready.</strong><p>Optional IDs and QR codes help you find a registered box again. Print these from your laptop when useful.</p></div><a href="/print/garage-labels-avery-5163-crates-1-5.pdf" target="_blank" rel="noreferrer">Avery 5163/8163 · Crates 1–5, two each ↓</a><a href="/print/garage-labels-avery-5163-crates-6-32.pdf" target="_blank" rel="noreferrer">Avery 5163/8163 · Remaining crates 6–32 ↓</a><a href="/print/garage-container-labels.pdf" target="_blank" rel="noreferrer">Download 32 blank labels ↓</a><small>C-001–C-032 · 8 per Letter page · Print at 100% / actual size. Match any existing IDs before sticking.</small></section>
      </details>
