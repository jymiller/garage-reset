...
export function App() {
  ...
  const [captureKind, setCaptureKind] = useState<'crate' | 'parking' | 'measurement' | 'placement'>('crate')
  ...
  function goTo(next: Tab) { if(next===tab && !labelCode)return; if(next==='discover'){setCaptureLabelCode(null);setCapturePhotoRole(null);setCaptureKind('crate');setCaptureCrateId(null);setCaptureVisit(n=>n+1)} setLabelCode(null); setCrateFocus(null); setPlayCrateId(null); navigate(next) }
  ...
  function capture(kind: 'crate' | 'parking' | 'measurement' | 'placement', crateId: string|null = null) {
  ...
  function captureLabel(code: string, role: 'outside'|'contents') { setCaptureLabelCode(code);setCapturePhotoRole(role);setCaptureKind('crate');setCaptureCrateId(null);setCaptureVisit(n=>n+1);navigate('discover') }
