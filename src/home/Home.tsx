...
export function Home({ onNavigate, onOpenCrate }: { onNavigate: (tab: Tab) => void; onOpenCrate: (id: string, step: 'locate' | 'sort' | 'repack') => void }) {
  ...
  return <main className="reset-home">
    <header className="home-heading"><div><p className="home-eyebrow">GARAGE RESET / BASE CAMP</p><h1>Let’s make<br className="home-mobile-break"/> some room.</h1><p>Small missions. Visible progress. Your garage back.</p></div><div className="home-level" aria-label={`Photo mission level ${score.level}`}><span>LEVEL</span><strong>{score.level}</strong><span>{score.points} XP</span></div></header>
    ...
    <div className="home-grid">
      ...
      <section className="home-how" aria-labelledby="home-how-title"><div className="home-section-heading"><div><p className="home-eyebrow">HOW TO PLAY</p><h2 id="home-how-title">Your next 10 minutes.</h2></div><span>One round is enough.</span></div><ol><li><span>01</span><div><h3>Show the starting point</h3><p>Choose a small area. Use <b>Take a photo</b> for a before shot.</p></div></li><li><span>02</span><div><h3>Give every thing a direction</h3><p>Put it away, bag it, donate it, or ask the owner. Count as you go.</p></div></li><li><span>03</span><div><h3>Capture the win</h3><p>Take the after shot, check both car paths, and finish for <b>100 XP</b>.</p></div></li></ol></section>
      ...
      <footer className="home-footer"><span>Phone for doing. Laptop for planning.</span><button onClick={() => onNavigate('more')}>Help & more tools <span aria-hidden="true">↗</span></button></footer>
    </div>
  </main>
}
