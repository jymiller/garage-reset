...
import { PhotoAnalysis } from '../analysis/PhotoAnalysis'
...
import { PhotoBatchCapture } from './PhotoBatchCapture'
import type { PhotoBatchSelection } from './PhotoBatchCapture'
import type { PhotoBatchContext } from './photoBatch'
import { OriginalPhotoLinks } from './OriginalPhotoLinks'
...
export function QuickCapture({ generalPhotoRequest = 0, initialKind = 'general', initialCrateId = null, initialView = 'capture', initialLabelCode = null, initialPhotoRole = null, onNavigate, onOpenCrate, onLabelQuest }: {
  ...
  const [batchSelection, setBatchSelection] = useState<PhotoBatchSelection | null>(null)
  ...
  function batchContext(): PhotoBatchContext {
    return { kind: draft.kind, crateId: draft.crateId, labelCode: draft.labelCode, photoRole: draft.photoRole,
      location: draft.location, helperId: readHelperPlayerId(), measurementEnabled: draft.measurementEnabled,
      measurementLabel: draft.measurementLabel, measurementValue: draft.measurementValue, measurementUnit: draft.measurementUnit }
  }
  function selectPhotos(files: File[]) {
    if (!files.length || busyRef.current || record) return
    if (files.length === 1) { selectFile(files[0]); return }
    if (draft.photo || draft.fileName) { setMessage('Finish saving the current photo first. Then choose your batch.'); return }
    const groups: PhotoBatchSelection['groups'] = [{ files, context: batchContext() }]
    setBatchSelection({ id: crypto.randomUUID(), groups })
  }
  function batchConsumed(id: string) {
    if (batchSelection?.id !== id) return
    if (hasContent(draftRef.current)) { setBatchSelection(null); return }
    forgetDraft(draftRef.current.id)
    const next = freshDraft(draft.kind, draft.crateId, draft.labelCode, draft.photoRole)
    draftRef.current = next; setDraft(next); setLocalPreview(null); fileRef.current = null
    setBatchSelection(null); setRestored(false); setMessage('')
  }
  function nextPhoto() {
  ...
  return <main className="quick-capture">
    ...
    {(workspace.conflict || workspace.error || workspace.status === 'offline' || workspace.storageError || draftStorageError) && <section className="quick-alert" aria-label="Save status">
      ...
      <div className="quick-button-row">{!workspace.conflict && <button type="button" className="quick-button" disabled={workspace.status === 'saving' || workspace.status === 'connecting'} onClick={workspace.retry}>{workspace.status === 'saving' ? 'Saving…' : 'Retry save'}</button>}<button type="button" className="quick-button secondary" onClick={() => { workspace.downloadDraft(); setBackedUp(true) }}>Download device draft</button>{draft.photo && !record && <button type="button" className="quick-button secondary" onClick={downloadPhotoDraft}>Download photo draft</button>}{workspace.conflict && <button type="button" className="quick-button secondary" disabled={!backedUp} onClick={() => { workspace.useSharedVersion(); setMessage('Your photo form is preserved. When the shared version finishes loading, Save photo to add just this find.'); }}>Load shared version</button>}</div>
    ...
    <PhotoBatchCapture workspace={workspace} selection={batchSelection} context={batchContext()} hidden={view !== 'capture'} onConsumed={batchConsumed} onActiveChange={setBatchActive} />
    {view === 'capture' && !batchActive && !batchSelection && <div className="quick-layout">
      <section className="quick-workbench" aria-label="Capture a useful photo">
        {!record ? <>
          ...
          <input ref={libraryInput} className="quick-file-input" type="file" accept="image/*,.heic,.heif" multiple aria-label="Choose garage photos" onChange={event => { selectPhotos(Array.from(event.target.files ?? [])); event.target.value = '' }} />
