...
const modes: { kind: Kind; title: string; heading: string; description: string; icon: 'crate' | 'garage' | 'measure' | 'placement' }[] = [
...
function readDraft(): Draft | null {
  try {
    ...
    if (!d || d.version !== 1 || typeof d.id !== 'string' || !d.id || d.id.length > 120
      || !['crate', 'parking', 'measurement', 'placement'].includes(d.kind) || !Number.isFinite(d.createdAt) || d.createdAt < 0 || d.createdAt > 8.64e15
...
export function QuickCapture({ initialKind = 'crate', initialCrateId = null, initialView = 'capture', initialLabelCode = null, initialPhotoRole = null, onNavigate, onOpenCrate, onLabelQuest }: {
  initialKind?: Kind; initialCrateId?: string | null; initialView?: 'capture' | 'collection'; initialLabelCode?: string | null; initialPhotoRole?: 'outside' | 'contents' | null; onLabelQuest?: (code?: string) => void; onNavigate: (tab: Tab) => void; onOpenCrate?: (id: string) => void;
}) {
  ...
  return <main className="quick-capture">
    <header className="quick-header"><span className="quick-header-icon"><GarageIcon name="missions" /></span><div><p className="quick-eyebrow">ONE USEFUL FIND AT A TIME</p><h1>{view === 'collection' ? 'Photos & measurements.' : 'Help with a photo.'}</h1><p>{view === 'collection' ? 'What we’ve seen and measured, ready for the next decision.' : 'One photo is enough to start.'}</p></div></header>
    {view === 'collection' && <div className="quick-view-tabs" role="group" aria-label="Photo workspace view"><button type="button" aria-pressed={false} onClick={() => { setView('capture'); window.scrollTo({ top: 0, behavior: 'auto' }) }}>Take a photo</button><button type="button" aria-pressed={true} onClick={openGallery}>Collected photos</button></div>}
    <div className={`quick-sync${ready ? ' is-shared' : ''}`} role="status"><span className="quick-sync-dot" /><span>{workspace.dirty && workspace.status === 'shared' ? 'Photo recorded on this device · waiting to share' : statuses[workspace.status]}</span><button type="button" onClick={openGallery}>{observations.length} useful {observations.length === 1 ? 'find' : 'finds'}</button></div>
    ...
    {view === 'capture' && <div className="quick-layout">
      <section className="quick-workbench" aria-label="Capture a useful photo">
        {!record ? <>
          ...
          {!draft.labelCode && <div className="quick-modes" role="group" aria-label="What to photograph">{modes.map(option => <button type="button" key={option.kind} disabled={formLocked} aria-pressed={draft.kind === option.kind} onClick={() => { change({ kind: option.kind, measurementEnabled: option.kind === 'measurement' || draft.measurementEnabled }); setPresetTip('') }}><GarageIcon name={option.icon} /><span>{option.title}</span></button>)}</div>}
          <div className="quick-prompt"><h2>{draft.labelCode ? `${draft.labelCode} · ${draft.photoRole === 'contents' ? 'Contents photo' : 'Outside photo'}` : mode.heading}</h2><p>{draft.labelCode ? draft.photoRole === 'contents' ? `Open just ${draft.labelCode} and show what is inside. Include its label in the view if you can; keep the parking boundary clear.` : `Show the ${draft.labelCode} label attached to the outside of the container, plus where the container lives.` : mode.description}</p></div>
          ...
          {photo && <figure className="quick-preview"><img src={photo} alt="Your photo draft" /><figcaption>{draft.photo ? 'Photo uploaded privately · finish with Save photo' : 'Photo selected · ready to add details'}</figcaption></figure>}
          <div className="quick-camera-actions"><button type="button" className="quick-button quick-camera" disabled={formLocked} onClick={() => cameraInput.current?.click()}><GarageIcon name="missions" />{photo ? 'Retake photo' : 'Take photo'}</button><button type="button" className="quick-button secondary" disabled={formLocked} onClick={() => libraryInput.current?.click()}>Choose from photos</button></div>
          ...
          {(photo || draft.fileName) && <form onSubmit={event => { event.preventDefault(); void savePhoto() }}>
            <fieldset className="quick-fields" disabled={formLocked}>
              <label>Where is this? <span className="quick-optional">Optional</span><input value={draft.location} maxLength={160} placeholder={draft.kind === 'placement' ? 'Where the object is now' : 'Right rack · middle shelf'} onChange={event => change({ location: event.target.value })} /></label>
              <label>What did you notice? <span className="quick-optional">Optional</span><textarea value={draft.notes} rows={3} maxLength={4000} placeholder={draft.kind === 'placement' ? 'Garbage bin: moved from the car space to beside the entrance…' : draft.kind === 'parking' ? 'Which car goes where? What is blocking the route?' : 'A few words about the contents or what needs a closer look…'} onChange={event => change({ notes: event.target.value })} /></label>
              ...
              {draft.measurementEnabled && <div className="quick-measurement"><label>What did you measure? <span className="quick-optional">Name both endpoints</span><input value={draft.measurementLabel} maxLength={160} placeholder="Rack wall to white line at the entrance end" onChange={event => change({ measurementLabel: event.target.value })} /></label><div className="quick-reading-fields"><label>Your reading<input inputMode="decimal" type="number" min="0" max="1000000" step="any" value={draft.measurementValue} placeholder="Leave blank for now" onChange={event => change({ measurementValue: event.target.value.slice(0, 100) })} /></label><label>Unit<select value={draft.measurementUnit} onChange={event => change({ measurementUnit: event.target.value as Unit })}><option value="cm">cm</option><option value="m">m</option><option value="in">inches</option><option value="ft">feet</option></select></label></div><p className="quick-small">Enter a reading you measured yourself. One pair of endpoints per photo. You can save the photo now and record the reading in another capture later.</p></div>}
              <details className="quick-link-crate" open={Boolean(draft.crateId) || undefined}><summary>Link an existing crate <span className="quick-optional">Optional</span></summary><label>Crate record<select value={draft.crateId ?? ''} onChange={event => change({ crateId: event.target.value || null })}><option value="">No crate link</option>{draft.crateId && !linkedCrate && <option value={draft.crateId}>Previously selected crate · unavailable</option>}{availableCrates.map(crate => <option key={crate.id} value={crate.id}>{crate.code} · {crate.name}</option>)}</select></label><p className="quick-small">This adds a photo observation. It does not create a crate or change its contents or volume.</p></details>
            </fieldset>
            ...
            {!ready && <p className="quick-small">You can take a photo and add notes now. Connect to the shared workspace before saving.</p>}
          </form>}
          {!photo && !draft.fileName && <p className="quick-capture-note">Take one photo first. Details and measurements are optional.</p>}
        </> : <section className="quick-saved" aria-live="polite"><span className="quick-saved-icon"><GarageIcon name={confirmed ? 'trophy' : 'missions'} /></span><p className="quick-eyebrow">{confirmed ? 'A LITTLE MORE OF THE GARAGE, UNDERSTOOD' : 'YOUR PHOTO DRAFT IS KEPT HERE'}</p><h2>{confirmed ? 'Helpful photo saved.' : 'Waiting to share your photo.'}</h2><p>{confirmed ? `${observations.length} useful ${observations.length === 1 ? 'find' : 'finds'} collected. Ready to review on your phone or laptop.` : 'Keep this page open while it saves. If another device changed the workspace, use the review controls above.'}</p><figure className="quick-preview"><img src={record.photo} alt={record.location || 'The photo you captured'} /></figure>{record.measurement && <div className="quick-saved-reading"><strong>{reading(record)}</strong><span>{record.measurement.label}</span><small>Measured by you</small></div>}{record.kind === 'measurement' && !record.measurement && <p>Photo saved as a reference. The reading is still pending.</p>}<div className="quick-button-row">{record.labelCode && onLabelQuest && <button type="button" className="quick-button" disabled={!confirmed} onClick={() => onLabelQuest(record.labelCode)}>Back to this crate</button>}<button type="button" className="quick-button" disabled={!confirmed} onClick={nextPhoto}>{record.labelCode ? `Take another ${record.photoRole === 'contents' ? 'contents' : 'outside'} photo` : 'Take another photo'}</button><button type="button" className="quick-button secondary" onClick={openGallery}>Review collected photos</button></div>{confirmed && <p className="quick-photo-points">{workspace.data.photoAwards?.some(award=>award.observationId===record.id) ? '25 photo points awarded.' : 'Useful photos earn 25 points after John reviews them. Photo points do not earn cash.'} <button type="button" className="quick-text-button" onClick={()=>onNavigate('score')}>See Score →</button></p>}{record.crateId && onOpenCrate && <button type="button" className="quick-text-button" onClick={() => onOpenCrate(record.crateId!)}>Open linked crate →</button>}</section>}
        {message && !confirmed && <p className="quick-message" role="status">{message}</p>}
      </section>
      <aside className="quick-side"><HelperIdentity workspace={workspace} selectedPlayerId={helperId} onSelectPlayer={setHelperId} /><div className="quick-tip"><GarageIcon name="shelf" /><h2>Keep the next step small.</h2><p>One open crate. One clear view. One reading you can trust.</p><p>Keep the white parking boundary and both car routes clear while you work.</p></div><div className="quick-other-actions"><button type="button" className="quick-button secondary" onClick={() => onNavigate('play')}><GarageIcon name="missions" />Start a cleanup mission</button><button type="button" className="quick-button secondary" onClick={() => onNavigate('layout')}><GarageIcon name="garage" />Open garage plan</button></div></aside>
    </div>}
    {view === 'collection' && <section className="quick-gallery" ref={galleryRef} aria-labelledby="quick-gallery-heading"><div className="quick-gallery-heading"><div><p className="quick-eyebrow">COLLECTED FOR THE NEXT DECISION</p><h2 id="quick-gallery-heading">Useful finds <span>{observations.length}</span></h2></div><label>Show<select value={galleryFilter} onChange={event => { setGalleryFilter(event.target.value as Kind | 'all'); setGalleryLimit(12) }}><option value="all">All photos</option>{modes.map(option => <option key={option.kind} value={option.kind}>{option.title}</option>)}</select></label></div>
