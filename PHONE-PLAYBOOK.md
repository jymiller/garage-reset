...
Start at **Home → Add photo**, or tap **Add photo** in the navigation. Take a new picture or choose one already on your phone. Anything that helps explain the garage is welcome: a shelf, loose object, box contents, parking space, tape reading, or something whose location changed. You do not need to pick a category, label a crate, set up inventory, choose a player name, or start a timer.

The more specific **Crate**, **Object moved**, **Both cars**, and **Measure** options remain available when their prompts help. A closed box is useful: show it where it lives and open the lid only if it is easy. The longer before-and-after cleanup rounds are available separately through **Start a cleanup mission**, or **More → Cleanup missions**.

## Save one useful photo

1. Choose **Take photo** or **Choose from photos**.
2. Add a short note if useful: “New home for the garbage bin” or “Unsure what's in this box.” You can leave it empty. Your phone keyboard’s microphone can dictate the note.
3. Leave measurements and the existing-crate link empty unless you have something to record.
4. Tap **Save photo**. Wait for **Photo saved.** before treating it as shared.
5. Choose **Add another photo** or **See photos**.
...
1. Open **Home → Label crates**. The same tool is in **More → Label crates** and **Crates → Label crates**.
2. Put matching copies of the same ID on the front or side of an easy-to-reach container and on its lid. Enter the ID and tap **Choose**. You can type 1 for C-001. Choose your name, then confirm **Front / side sticker** and **Lid sticker** as you place each one. Each earns **25 points**, once per position for that crate.
3. Alternatively, scan the printed QR with the iPhone Camera and tap its link. It opens **Crates** for that ID; choose **Label crates**.
...
Choose **Home → Show where something moved**, or **Add photo → Object moved**. Photograph the bin, box, or object with its surroundings and any ID. Note what moved, where it was, and where it is now. This records useful evidence of the new location; it does not move the object in the 3D model automatically.

## Show the parking and measure a useful gap

Choose **Home → Make room for both cars**, or **Add photo → Both cars**. Photograph the actual cars in their normal parked positions. Take a wide view from the entrance, then another from the rear. Include the white parking boundary, the routes around the cars, and the room for opening doors. Note which car goes where and what blocks a route.

Choose **Home → Measure one space**, or **Add photo → Measure**, for a shelf depth, clear route width, or white-boundary measurement. Show both endpoints and the tape reading. Under **Add a measurement**, name the endpoints, enter **Your reading**, and choose the **Unit**. For example: “Rack wall to white line at the entrance end,” followed by the number actually read from the tape.
...
**Your name** appears on Home and beside or below photo capture. Select your name, or choose **Add name**, enter a first name or nickname, and tap **Save**. The choice is remembered on this device and also appears on Score. **No name yet** keeps the photo flow available.
